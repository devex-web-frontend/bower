...
SvnResolver.prototype._export = function () {
	var exportFolder = [this._source];

	exportFolder.push(FOLDER_MAP[this._resolution.type]);

	if (this._resolution.target !== 'trunk') {
		exportFolder.push(this._resolution.target);
	}
	...
	return cmd('svn', ['export', exportFolder.join('/'), this._tempDir, '--force']);
};
